import { CircleGeometry, DoubleSide, Group, Mesh, MeshLambertMaterial, Vector3, Euler, Color, LineBasicMaterial, LineSegments, MeshPhongMaterial, SphereGeometry, SRGBColorSpace, TextureLoader, BufferGeometry, CylinderGeometry, Matrix4, Object3D, CubicBezierCurve3, Curve, Line, NormalBlending, QuadraticBezierCurve3, ShaderMaterial, TubeGeometry, Vector2, BackSide, BufferAttribute, Float32BufferAttribute, MeshBasicMaterial, AmbientLight, DirectionalLight, REVISION } from 'three';
...
var PointsLayerKapsule = Kapsule({
  ...
  update: function update(state) {
    ...
    var initAltitudeAccessor = accessorFn(state.pointInitAltitude);
    ...
    function updateObj(obj, d) {
      ...
      if (Object.keys(targetD).some(function (k) {
        ...
        if (state.pointsMerge || !state.pointsTransitionDuration || state.pointsTransitionDuration < 0) {
          ...
          // animate
          var delay = state.pointsTransitionDuration * initAltitudeAccessor(d);
          setTimeout(function () {
            new TWEEN.Tween(currentTargetD).to(targetD, state.pointsTransitionDuration - delay).easing(TWEEN.Easing.Quadratic.InOut).onUpdate(applyUpdate).start();
          }, delay);
        }
      }
      if (!state.pointsMerge) {
...
var linkedPointsLayerProps = Object.assign.apply(Object, _toConsumableArray(['pointsData', 'pointLat', 'pointLng', 'pointColor', 'pointAltitude', 'pointInitAltitude', 'pointRadius', 'pointResolution', 'pointsMerge', 'pointsTransitionDuration'].map(function (p) {
...
var globe = Kapsule({
  ...
  stateInit: function stateInit(_ref6) {
    var rendererConfig = _ref6.rendererConfig,
      ...
      globeInitConfig = _objectWithoutProperties$1(_ref6, _excluded);
